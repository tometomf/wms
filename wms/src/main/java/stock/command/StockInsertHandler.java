...
public class StockInsertHandler implements CommandHandler {
    ...
    private String processForm(HttpServletRequest req, HttpServletResponse res) {
        Stock stock = insertService.getstockNo(); // 신규 재고번호 조회 / 新しい在庫番号を取得
    ...
    private String processSubmit(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        String stockNoStr = req.getParameter("stockNo");
        String itemCd = req.getParameter("itemCd");
        String qtyStr = req.getParameter("qty");
        String wareCd = req.getParameter("wareCd");
        ...
        try {
            ...
            try {
                stock.setStock_No(Integer.parseInt(stockNoStr.trim()));
            ...
            // 나머지 필드 세팅 / その他のフィールドを設定
            stock.setItem_Cd(itemCd);
            stock.setWare_Cd(wareCd);
            ...
            errors.put("insertError", Boolean.TRUE); // 등록 실패 / 登録失敗
            req.setAttribute("stockNo", insertService.getstockNo());
            return FORM_VIEW;
        }
    }
